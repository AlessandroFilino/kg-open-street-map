...
GRAPH_EXISTS=$(echo "SPARQL ASK { GRAPH <$GRAPH_NAME> { ?s ?p ?o } };" | isql-vt -H kg-open-street-map-virtuoso-1 -P $VIRTUOSO_PSW) 
...
if [ "$GRAPH_EXISTS" -eq 1 ];
    then
        ...
        echo "SPARQL CLEAR GRAPH <$GRAPH_NAME>;" | isql-vt isql-vt -H kg-open-street-map-virtuoso-1 -P $VIRTUOSO_PSW
    fi

# Rimuoviamo il file dalla load list per ricaricarlo
echo "DELETE FROM DB.DBA.LOAD_LIST WHERE ll_file='$RELATION_NAME/$RELATION_NAME.n3';" | isql-vt isql-vt -H kg-open-street-map-virtuoso-1 -P $VIRTUOSO_PSW
